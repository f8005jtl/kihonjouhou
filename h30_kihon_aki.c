...
/*
    //printf("・train_info\n");
    //printf(" | 列車  ");for(int k=0;k<9;k++)printf(" | 列車%d",k);printf(" | \n");
    //printf(" |number ");for(int k=0;k<9;k++)printf(" |%4d  ",trains[k].number);//printf(" | \n");
    //printf(" |dest  ");for(int k=0;k<9;k++)printf(" |%4d  ",nullcheck(trains[k].dest)?trains[k].dest->n:999);printf(" | \n");

  // 区間ノードの生成
  struct block_info kukan0 = {NULL,NULL,NULL,RED};
  struct block_info kukan1 = {NULL,NULL,NULL,RED};
  struct block_info kukan2 = {NULL,NULL,NULL,GREEN};
  struct block_info kukan3 = {NULL,NULL,NULL,RED};
  struct block_info kukan4 = {NULL,NULL,NULL,RED};
  struct block_info kukan5 = {NULL,NULL,NULL,GREEN};
  struct block_info kukan6 = {NULL,NULL,NULL,GREEN};
  struct block_info kukan7 = {NULL,NULL,NULL,GREEN};
  struct block_info kukan8 = {NULL,NULL,NULL,RED};

  // 列車ノードの生成
  struct train_info train0 = {0,NULL};
  struct train_info train1 = {1,NULL};
  struct train_info train2 = {2,NULL};
  struct train_info train3 = {3,NULL};
  struct train_info train4 = {4,NULL};

  // 各ノードに「次の場所」を追加(区間)
  kukan0.next[0] = &kukan2;
  kukan1.next[0] = &kukan2;
  kukan2.next[0] = &kukan3;
  kukan3.next[0] = &kukan4;
  kukan3.next[1] = &kukan7;
  kukan4.next[0] = &kukan5;
  kukan4.next[1] = &kukan6;
  kukan7.next[0] = &kukan8;
...
*/
    ...
    for(int i=0;i<9;i++){
      printf(" | 区間%d",i);
      //printf("区間%d | train=&列車%3d | next[0]=&区間%3d next[1]=&区間%3d | signal=%s \n",i,nullcheck(blocks[i].train)?blocks[i].train->number:999,nullcheck(blocks[i].next[0])?blocks[i].n:999,nullcheck(blocks[i].next[1])?blocks[i].n:999,blocks[i].signal ? "GREEN" : "RED");
    }*/
      /*
    for(j=0;j<2;j++){
      if(block->next[j]==NULL){
      ...
      if(block->next[i]->signal==RED){// 信号が赤
        continue;
      }
      if(find_block(block->next[j],block->train->dest) == 1){// 終着区間か(同じ区間か)
        block->next[j]->train=block->train;
